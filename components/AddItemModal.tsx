...
export const AddItemModal = ({
    ...
    const handleBlurTitle = async () => {
        // AIによるCategory分類を一時的に停止するためコメントアウト
        /*
        if (!newItem) return;
    ...
    return (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            ...
            <div className="bg-white p-8 rounded-3xl w-full max-w-sm shadow-2xl relative z-10 text-black">
                <h2 className="text-xl font-bold mb-4 text-center">New Bucket List Item</h2>
                <input
                ...
                <div className="mb-4">
                    ...
                    {isSuggesting && <span className="text-xs text-blue-500 mt-1 block">✨ AI predicting category...</span>}
                </div>
                ...
                <div className="flex gap-2">
                    <button type="button" onClick={() => setIsOpen(false)} className="flex-1 text-gray-500 py-2">Cancel</button>
                    <button type="button" onClick={onAdd} className="flex-1 bg-blue-500 text-white py-3 rounded-xl font-bold hover:bg-blue-600 transition-colors">Add</button>
                </div>
            </div>
        </div>
    );
